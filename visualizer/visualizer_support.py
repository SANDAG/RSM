...
def intrazonal_vmt_aggregation(vmt_summary_df_file_name, intrazonal_trips_file_name, intrazonal_vmt_file_name, scenario):

    scenario_dir = os.path.join("SimWrapper\\data\\processed", scenario)
    vmt_summary_df = pd.read_csv(os.path.join(scenario_dir, vmt_summary_df_file_name))
    intrazonal_vmt_df = pd.read_csv(os.path.join(scenario_dir, intrazonal_trips_file_name))
...
if __name__ == "__main__":
    ...
    intrazonal_trips_file_name = config['inputs']['intrazonal_trips_file_name']
    rsm_scenario_list = config['parameters']['rsm_scenario_list']
    base_scenario = config['parameters']['base_scenario']
    intrazonal_vmt_file_name = config['outputs']['intrazonal_vmt_file_name']
    ...
    for scenario in (rsm_scenario_list + [base_scenario]):
        intrazonal_vmt_aggregation(vmt_summary_file_name, intrazonal_trips_file_name, intrazonal_vmt_file_name, scenario)

    combine_scenarios_summary(rsm_scenario_list + [base_scenario], mode_summary_file_name, vmt_summary_file_name, intrazonal_vmt_file_name, compared_scenarios_dir)
