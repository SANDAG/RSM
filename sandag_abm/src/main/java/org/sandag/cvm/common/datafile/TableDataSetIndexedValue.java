...
public class TableDataSetIndexedValue implements TableDataSetIndex.ChangeListener, java.io.Serializable, Cloneable {
    ...
    /* (non-Javadoc)
     * @see org.sandag.cvm.common.datafile.TableDataSetIndex.ChangeListener#indexChanged()
     */
    ...
    public boolean hasValidLinks(TableDataSetCollection collection) {
    	try {
    		updateLinks(collection);
    		if (lastRowNumbers.length==0) return false;
    		if (lastRowNumbers.length>1 && valueMode == SINGLE_VALUE_MODE) {
    			return false;
    		}
    		return true;
    ...
    public boolean hasValidLinks() {
    	return hasValidLinks(myLastCollection);
    }
