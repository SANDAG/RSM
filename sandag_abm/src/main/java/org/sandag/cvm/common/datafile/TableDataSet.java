...
public class TableDataSet implements DataTypes, Serializable {
    ...
    */
     ...
     * not STRING then the numeric value will be converted to string before
     * it is returned.
     *
     */
    ...
    public boolean getBooleanValueAt(int row, int column) {
        ...
        if (use1sAnd0sForTrueFalse ) {
            if (boolString.equalsIgnoreCase("1")) return true;
            if (boolString.equalsIgnoreCase("0")) return false;
        }
        /* ABDEL M.:I added boolString.equalsIgnoreCase("t") and boolString.equalsIgnoreCase("f") to the condition below.
        The method getStringValueAt(row,column) above returns t or f rather than true or false. It is not the best way to fix the issue but this is a work around
        for now till John A. comes and check it.
        */
        if (boolString.equalsIgnoreCase("true") || boolString.equalsIgnoreCase("t")) return true;
        if (boolString.equalsIgnoreCase("false") ||  boolString.equalsIgnoreCase("f")) return false;
     ...
     *
     * @param newColumn the array of data to append to TableDataSet (float[], int[], double[] or String[])
