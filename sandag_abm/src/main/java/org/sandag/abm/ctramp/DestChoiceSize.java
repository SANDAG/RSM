        ...
        implements Serializable
{
    ...
    public void balanceSizeVariables(int[][] originsByHomeMgra)
    {
        ...
        for (int i = 0; i < numSegments; i++)
        {
            ...
            logger.info(String.format("    %-6d  %-55s:  %10.1f", i, segmentString, sumScaled[i]));
        }
        logger.info(String.format("    %-6s  %-55s:  %10.1f", " ", "Total", segmentSum));
        logger.info("");
        ...
        for (int r = 0; r < maxSize.length; r++)
        {
            ...
            for (int i = 0; i < numSegments; i++)
            {
                ...
                for (int j = 1; j <= maxMgra; j++)
                {
                    ...
                    if (scaledSize[i][j] > minRange && scaledSize[i][j] <= maxSize[r])
                    {
                        ...
                        } else
                        {
                            ...
                            double relDiff = Math.abs(scaledSize[i][j]
                ...
                {
                    float pct = 0.0f;
                    if (tot > 0) pct = (float) (100.0 * freqs[k] / tot);
            ...
            {
                float pct = 0.0f;
                if (tot > 0) pct = (float) (100.0 * tots[k] / tot);
            ...
            int[][] modeledDestinationLocationsByDestMgra, String mandatoryType)
    {
        ...
        for (int i = 0; i < numSegments; i++)
        {
            ...
            tableHeadings.add(String.format("%s_sizeAdjOriginal", segmentString));
        ...
        try
        {
            String newFilename = this.dcShadowOutputFileName.replaceFirst(".csv", "_"
                    + mandatoryType + "_" + iteration + ".csv");
    ...
    public void restoreShadowPricingInfo(String fileName)
    {
        ...
        {
            logger.error("exception reading saved shadow price file: " + fileName
                    + " from previous model run.", e);
        }
