        ...
        implements Serializable
{
    ...
    // these are public because CtrampApplication creates a
    // ChoiceModelAppplication in order to get the alternatives for a log report
    ...
    public void applyModel(Household household)
    {
        ...
        // this household does not make joint tours if the CDAP pattern does not
        // contain "j".
        ...
        try
        {
            ...
            // joint tour frequency choice is not applied to a household unless
            // it has:
            // 2 or more persons, each with at least one out-of home activity,
            // and at
            ...
            // if it's not a valid household for joint tour frequency, keep
            // track of
            ...
            // set the household id, origin taz, hh taz, and debugFlag=false in
            // the dmu
            dmuObject.setHouseholdObject(household);

            // set the accessibility values needed based on auto sufficiency
            // category for the hh.
    ...
    private void jointTourParticipation(Tour jointTour)
    {
        ...
        // define an ArrayList to hold indices of person objects participating
        // in the joint tour
        ArrayList<Integer> jointTourPersonList = null;

        // make sure each joint tour has a valid composition before going to the
        // next one.
        ...
        while (!validParty)
        {
            ...
            for (int p = 1; p < persons.length; p++)
            {
                ...
                // if person type is inconsistent with tour composition,
                // person's
                ...
                // adults only in joint tour
                    case 1:
                        if (persons[p].getPersonIsAdult() == 1)
                        {
                            ...
                            // particpate is alternative 1, not participating is
                            // alternative 2.
                            if (chosen == 1)
                            {
            ...
            // done with all persons, so see if the chosen participation is a
            // valid
    ...
    /**
     * creates the tour objects in the Household object given the chosen joint
     * tour frequency alternative.
     *
     * @param chosenAlt
     */
    ...
    public static void main(String[] args)
    {
        ...
        /*
         * Use this block to instantiate a UEC for the joint freq/comp model and
         * a UEC for the joint participate model. After checking the UECs are
         * instantiated correctly (spelling/typos/dmu methods implemented/etc.),
         * test model implementation. String uecFileDirectory = propertyMap.get(
         ...
         * File uecFile = new File(uecFileDirectory + propertyMap.get(
         * UEC_FILE_PROPERTIES_TARGET )); UtilityExpressionCalculator uec = new
         * UtilityExpressionCalculator(uecFile, 1, 0, propertyMap,
         * (VariableTable)dmuObject);
         * System.out.println("Jount tour freq/comp choice UEC passed.");
         *
         * uec = new UtilityExpressionCalculator(uecFile, 2, 0, propertyMap,
         * (VariableTable)dmuObject);
