 ...
 */
public class StopDepartArrivePeriodModel
        implements Serializable
{
     ...
     *            - model definitions helper class
     */
    public StopDepartArrivePeriodModel(HashMap<String, String> propertyMap,
    ...
    private void readLookupProportions(String propsLookupFilename)
    {
        ...
        int lastInterval = modelStructure
                .getTimePeriodIndexForTime(ModelStructure.LAST_TOD_INTERVAL_HOUR);
        ...
        // tourpurp isInbound interval trip p1-p40 (alternative interval
        // proportions)

        // populate the outProportionsMaps and inProportionsMaps arrays of maps
        // from data in the TableDataSet.
        // when stops are generated, they can lookup the proportions for stop
        // depart or arrive interval determined
        // by tour purpose, outbound/inbound direction and interval of previous
        // trip. From these proportions,
        ...
        for (int i = 0; i < propsLookupTable.getRowCount(); i++)
        {
            ...
            int tourPrimaryPurposeIndex = (int) propsLookupTable.getValueAt(i + 1,
                    TOUR_PURPOSE_INDEX_COLUMN_HEADING);

            // get the half tour direction (0 for outbound or 1 for inbound)
            int direction = (int) propsLookupTable.getValueAt(i + 1,
            ...
            int firstPropColumn = propsLookupTable
                    .getColumnPosition(INTERVAL_1_PROPORTION_COLUMN_HEADING);

            // starting at this column, read the proportions for all TOD
            // interval proportions.
            // Create the array of proportions for this table record.
            for (int j = 1; j <= lastInterval; j++)
                proportions[tourPrimaryPurposeIndex][direction][todInterval][tripNumber][j] = propsLookupTable
                        .getValueAt(i + 1, firstPropColumn + j - 1);

        }

    }

    public double[] getStopTodIntervalProportions(int tourPrimaryPurposeIndex, int direction,
            int prevTripTodInterval, int tripNumber)
    {
