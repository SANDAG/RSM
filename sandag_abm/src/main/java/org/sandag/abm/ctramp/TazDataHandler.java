        ...
        implements TazDataIf, Serializable
{
    ...
    private static final String[] SUB_ZONE_NAMES                                     = {"none",
            "short", "long"                                                          };
    ...
    // zoneTableRow is a correspondence between zone number (i,e, origin taz or
    // dest
    // taz) and the TableDataSet row
    // in which those zonal attributes are stored.
    protected int[]               zoneTableRow;

    // indexToZone is a correspondence between 1-based zone index values
    // (1,...,numZones) and the zone values.
    protected int[]               indexToZone;

    // altToZone is a correspondence between DC alternative number
    // (1,...,numZones*numSubzones) and 1-based zone index (1,...,numZones)
    protected int[]               altToZone;

    // altToSubZone is a correspondence between DC alternative number
    // (1,...,numZones*numSubzones) and 0-based subzone index
    // (0,...,numSubZones-1)
    protected int[]               altToSubZone;

    // 1st dimension is indexed by zone number, 2nd dimension is walk segments -
    // e.g.
    // 0: no walk %, 1: shrt %, 2: long %.
    protected float[][]           zonalWalkPctArray;

    protected String[]            subZoneNames;

    // TODO: need to apply consistent zone number/zone table row/o & d zonal
    // index/
    // correspondence defined in this class
    // TODO: and make sure that proper indexing is applied in all models that
    // use
    // zone table data.
    ...
    public TazDataHandler(ResourceBundle rb, String projectDirectory)
    {
        ...
        walkPercentFileName = projectDirectory
                + ResourceUtil.getProperty(rb, PROPERTIES_WALK_SHARES_DATA);

        // locate the walk percentages data from the .properties file
        zonalAccessibilityFileName = projectDirectory
                + ResourceUtil.getProperty(rb, PROPERTIES_ZONAL_ACCESSIBILITIESS_DATA);
    ...
    public void setupTazDataManager()
    {
        ...
        // TODO how does mtc do walk percentages
        readWalkPercentagesFile(walkPercentFileName);

        // dimension the correspondence arrays that will contain the zone and
        // subzone, respectively, given the DC alternative index (1,...,numAlts)
    ...
    private TableDataSet readZonalData(String zoneDataFileName)
    {
        ...
        try
        {
            ...
            // set global max zone and num zones values, and make sure there are
            // no
            // duplicates.
        ...
        // store the row numbers for each zone so that zonal attributes can be
        // retrieved later using given a zone number
    ...
    private void readWalkPercentagesFile(String fileName)
    {
        ...
        zonalWalkPctArray = new float[3][NUM_ZONES + 1];
        ...
        if (fileName != null)
        {
            ...
            try
            {
                ...
                for (int j = 1; j <= wa.getRowCount(); j++)
                {
                    ...
                    zonalWalkPctArray[1][taz] = shrtArray[taz];
                    zonalWalkPctArray[2][taz] = longArray[taz];
                    zonalWalkPctArray[0][taz] = (float) (1.0 - (shrtArray[taz] + longArray[taz]));
                }
    ...
    private void readZonalAccessibilitiesFile(String fileName)
    {
        ...
        if (fileName != null)
        {
            ...
            try
            {
                ...
                int pkTransitRetailPosition = acc
                ...
                int pkTransitTotalPosition = acc
                ...
                int opTransitRetailPosition = acc
                ...
                int opTransitTotalPosition = acc
                ...
                int nonMotorizedRetailPosition = acc
                ...
                int nonMotorizedTotalPosition = acc
            ...
            {
                logger.fatal(String.format("exception caught reading accessibilities file: %s",
                        fileName), e);
            }
    ...
    private int getZoneFromAlt(int alt)
    {
        ...
        if (zone < 1 || zone > NUM_ZONES)
        {
            logger.fatal(String.format(
                    "invalid value for zone index = %d, determined for alt = %d.", zone, alt));
