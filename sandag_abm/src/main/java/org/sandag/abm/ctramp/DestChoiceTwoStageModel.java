...
public class DestChoiceTwoStageModel
{
    ...
    private transient Logger logger           = Logger.getLogger(DestChoiceTwoStageModel.class);
    ...
    public DestChoiceTwoStageModel(HashMap<String, String> propertyMap, int soaMaxSampleSize)
    {
    ...
    public void computeSoaProbabilities(int origTaz, int segmentTypeIndex)
    {
        ...
        for (int taz = 1; taz <= tdm.getMaxTaz(); taz++)
        {
            ...
            if (mgraArray == null) continue;

            if (sizeProbs[taz - 1].length == 0) continue;

            double tazProb = 0;
            if (taz > 1) tazProb = tazDistCumProbs[segmentTypeIndex][origTaz - 1][taz - 1]
                    - tazDistCumProbs[segmentTypeIndex][origTaz - 1][taz - 2];
            else tazProb = tazDistCumProbs[segmentTypeIndex][origTaz - 1][0];
        ...
        try
        {
            out = new PrintWriter(new BufferedWriter(new FileWriter(new File("distSoaProbs.csv"))));
    ...
    public void chooseSampleMgra(int sampleIndex, int origTaz, int sizePurposeIndex,
            int segmentTypeIndex, double rn, boolean debug)
    {
        ...
        if (mgraSizeProbs[segmentTypeIndex][chosenTazIndex].length == 0)
        {
            ...
            logger.error("Likely cause is an indexing bug.  sampleIndex=" + sampleIndex
                    + ", origTaz=" + origTaz + ", sizePurposeIndex=" + sizePurposeIndex
        ...
        // array
        int chosenMgraIndex = Util.binarySearchDouble(cumProbabilityLowerBound, tempMgraCumProbs,
                mgraArray.length, rn);
        ...
        if (debug)
        {
            ...
            logger.info(String.format(
                    "%-12d %10d %10.6f %16.8f %16.8f %18d %18.8f %18.8f %12d %18.8f", sampleIndex,
                    chosenTazIndex, rn, prevDistCumProb, cumDistProb, chosenMgraIndex,
                    prevSizeCumProb, cumSizeProb, chosenMgra,
                    ((cumSizeProb - prevSizeCumProb) * (cumDistProb - prevDistCumProb))));
        }

    }

    private void chooseSlcSampleMgraBinarySearch(int sampleIndex, int slcOrigTaz, int slcDestTaz,
            int slcSizeSegmentIndex, double rn, boolean debug)
    {
        ...
        if (slcSizeProbs[slcSizeSegmentIndex][chosenTazIndex].length == 0)
        {
            ...
            logger.error("This should not be the case.  If a TAZ was chosen, its TAZ Size > 0, so there should be at least one MGRA with size > 0 in the TAZ.");
            logger.error("Likely cause is an indexing bug.  sampleIndex=" + sampleIndex
                    + ", slcOrigTaz=" + slcOrigTaz + ", slcDestTaz=" + slcDestTaz
                    + ", slcSizeSegmentIndex=" + slcSizeSegmentIndex);
            throw new RuntimeException();
        }
        ...
        if (chosenTazIndex > 0)
        {
            tazProb = slcTazCumProbs[chosenTazIndex] - slcTazCumProbs[chosenTazIndex - 1];
    ...
    private void chooseSlcSampleMgraLinearWalk(int sampleIndex, int slcOrigTaz, int slcDestTaz,
            int slcSizeSegmentIndex, double rn, boolean debug)
    {
    ...
    public void chooseSample(int origTaz, int sizeSegmentIndex, int segmentTypeIndex,
            int numInSample, Random rand, boolean debug)
    {
        ...
        {
            chooseSampleMgra(i, origTaz, sizeSegmentIndex, segmentTypeIndex, rand.nextDouble(),
                    debug);
        }
    ...
    public void chooseSlcSample(int origTaz, int destTaz, int sizeSegmentIndex, int numInSample,
            Random rand, boolean debug)
    {
        ...
        for (int i = 0; i < numInSample; i++)
        {
            ...
            chooseSlcSampleMgraLinearWalk(i, origTaz, destTaz, sizeSegmentIndex, rand.nextDouble(),
                    debug);
        }
    ...
    public void setSlcSoaProbsAndUtils(double[][] slcTazDistExpUtils, double[][][] slcSizeProbs,
            double[][] slcTazSize)
    {
