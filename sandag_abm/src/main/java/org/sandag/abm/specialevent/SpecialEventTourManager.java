...
public class SpecialEventTourManager
{
     ...
     * Default Constructor.
     */
    public SpecialEventTourManager(HashMap<String, String> rbMap, TableDataSet eventData)
    {
        this.rbMap = rbMap;
    ...
    public void generateTours()
    {
        ...
        String incomeFile = Util.getStringValueFromPropertyMap(rbMap, "specialEvent.income.file");
        ...
        for (int i = 1; i <= eventData.getRowCount(); ++i)
        {
            ...
            for (int j = 0; j < attendance; ++j)
            {
                ...
                int income = chooseIncome(randomGenerator.nextDouble(), eventType);
                int partySize = choosePartySize(randomGenerator.nextDouble(), eventType);
    ...
    public void writeOutputFile(HashMap<String, String> rbMap)
    {
        ...
        String tourFileName = directory
                + Util.getStringValueFromPropertyMap(rbMap, "specialEvent.tour.output.file");
        String tripFileName = directory
                + Util.getStringValueFromPropertyMap(rbMap, "specialEvent.trip.output.file");
        ...
        String tripHeaderString = new String(
                "tourID,tripID,originMGRA,destinationMGRA,inbound,originIsTourDestination,destinationIsTourDestination,period,tripMode,boardingTap,alightingTap,set,valueOfTime");
     ...
     * @param writer
     */
    private void writeTrip(SpecialEventTour tour, SpecialEventTrip trip, int tripNumber,
            PrintWriter writer)
    {
