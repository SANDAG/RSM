        ...
        implements Serializable
{
    ...
    public static final int[]         MC_PURPOSE_SHEET_INDICES         = {-1, WORK_SHEET,
            UNIVERSITY_SHEET, SCHOOL_SHEET, MAINTENANCE_SHEET, MAINTENANCE_SHEET,
    ...
    public static final String[]      PURPOSE_CATEGORY_LABELS          = {"work", "university",
            "school", "maintenance", "discretionary", "subtour"        };
    public static final int[]         PURPOSE_CATEGORIES               = {-1, WORK_CATEGORY,
            UNIVERSITY_CATEGORY, SCHOOL_CATEGORY, MAINTENANCE_CATEGORY, MAINTENANCE_CATEGORY,
            MAINTENANCE_CATEGORY, DISCRETIONARY_CATEGORY, DISCRETIONARY_CATEGORY,
            DISCRETIONARY_CATEGORY, SUBTOUR_CATEGORY                   };
    ...
    protected static final int[]      DEFAULT_DEPART_INDICES           = {-1, EA_D, AM_D, MD_D,
            PM_D, EV_D                                                 };
    ...
    protected static final int[]      DEFAULT_ARRIVE_INDICES           = {-1, EA_A, AM_A, MD_A,
            PM_A, EV_A                                                 };
            ...
            int[] mgraSample, int[] departAvailable, int[] arriveAvailable, boolean chosenTodOnly)
    {
        ...
        mcDmuObject.setWorkTourModeIsSOV(odt[WORK_TOUR_MODE] == 1 || odt[WORK_TOUR_MODE] == 2 ? 1
                : 0);
        mcDmuObject.setWorkTourModeIsBike(odt[WORK_TOUR_MODE] == 10 ? 1 : 0);
        mcDmuObject.setWorkTourModeIsHOV(odt[WORK_TOUR_MODE] >= 3 || odt[WORK_TOUR_MODE] <= 8 ? 1
                : 0);
        ...
        for (int destMgra : mgraSetForLogsums)
        {
            ...
            for (int[] combo : departArriveCombinations)
            {
                ...
                if (!chosenTodOnly || isChosenDepartArriveCombo)
                {
                    ...
                    double logsum = logsumHelper.calculateTourMcLogsum(origMgra, destMgra,
                            departPeriod, arrivePeriod, mcModel, mcDmuObject);
    ...
    protected void calculateTripModeChoiceLogsums(HashMap<String, String> rbMap,
            ChoiceModelApplication mcModel, SandagTripModeChoiceDMU mcDmuObject, int[] odt,
            int[] mgraSample)
    {
        ...
        if (origMgra == 0 || destMgra == 0 || odt[TOUR_MODE] == 0) return;
        ...
        mcDmuObject
                .setEscortTour(odt[TOUR_PURPOSE] == ModelStructure.ESCORT_PRIMARY_PURPOSE_INDEX ? 1
                        : 0);
        ...
        if (mcModel == null || origMgra == 0) return;
        ...
        for (int sampleMgra : mgraSetForLogsums)
        {
            ...
            if ((odt[0] == debugEstimationFileRecord1 || odt[0] == debugEstimationFileRecord2))
            {
                ...
                nonManLogsumsLogger.info("IK Logsum calculation for estimation file record number "
                        + odt[0]);
                nonManLogsumsLogger.info("");
                nonManLogsumsLogger
                        .info("--------------------------------------------------------------------------------------------------------");
                ...
                nonManLogsumsLogger.info("origin taz = " + mgraManager.getTaz(origMgra));
                nonManLogsumsLogger.info("sample destination taz = "
                        + mgraManager.getTaz(sampleMgra));
                ...
                nonManLogsumsLogger.info("income category = " + odt[INCOME] + ", @income = "
                        + mcDmuObject.getIncome());
                nonManLogsumsLogger.info("adults = " + odt[ADULTS]);
                nonManLogsumsLogger.info("autos = " + odt[AUTOS]);
                nonManLogsumsLogger.info("hhsize = " + odt[HHSIZE]);
                nonManLogsumsLogger.info("gender = " + odt[FEMALE] + ", @female = "
           ...
           if ((odt[DIRECTION] == INBOUND_DIRCETION_CODE))
            ...
            } else logsumHelper.setDtwTripMcDmuAttributes(mcDmuObject, origMgra, sampleMgra,
                    departPeriod, mcDmuObject.getDmuIndexValues().getDebug());

            logsumHelper.setWtwTripMcDmuAttributes(mcDmuObject, origMgra, sampleMgra, departPeriod,
                    mcDmuObject.getDmuIndexValues().getDebug());

            double logsum = logsumHelper.calculateTripMcLogsum(origMgra, sampleMgra, departPeriod,
                    mcModel, mcDmuObject, nonManLogsumsLogger);
            ...
            if ((odt[0] == debugEstimationFileRecord1 || odt[0] == debugEstimationFileRecord2))
            {
                ...
                nonManLogsumsLogger.info("KJ Logsum calculation for estimation file record number "
                        + odt[0]);
                nonManLogsumsLogger.info("");
                nonManLogsumsLogger
                        .info("--------------------------------------------------------------------------------------------------------");
                ...
                nonManLogsumsLogger
                        .info("sample destination taz = " + mgraManager.getTaz(destMgra));
                nonManLogsumsLogger.info("final destination taz = " + mgraManager.getTaz(destMgra));
                nonManLogsumsLogger.info("depart interval = " + departPeriod);
                nonManLogsumsLogger.info("income category = " + odt[INCOME] + ", @income = "
                        + mcDmuObject.getIncome());
            ...
            logsum = logsumHelper.calculateTripMcLogsum(origMgra, sampleMgra, departPeriod,
                    mcModel, mcDmuObject, nonManLogsumsLogger);
    ...
    protected double[] calculateTripModeChoiceLogsumForEstimationRecord(
            HashMap<String, String> rbMap, ChoiceModelApplication mcModel,
            SandagTripModeChoiceDMU mcDmuObject, int[] odt, int sampleMgra)
    {
        ...
        if (mcDmuObject.getDmuIndexValues().getDebug())
        {
            ...
            slcSoaLogger.info("income category = " + odt[INCOME] + ", @income = "
                    + mcDmuObject.getIncome());
        ...
        if ((odt[DIRECTION] == INBOUND_DIRCETION_CODE))
        {
            logsumHelper.setWtdTripMcDmuAttributes(mcDmuObject, origMgra, sampleMgra, departPeriod,
                    mcDmuObject.getDmuIndexValues().getDebug());
        } else logsumHelper.setDtwTripMcDmuAttributes(mcDmuObject, origMgra, sampleMgra,
                departPeriod, mcDmuObject.getDmuIndexValues().getDebug());
        ...
        if ((odt[DIRECTION] == INBOUND_DIRCETION_CODE))
        {
            logsumHelper.setWtdTripMcDmuAttributes(mcDmuObject, sampleMgra, destMgra, departPeriod,
                    mcDmuObject.getDmuIndexValues().getDebug());
        } else logsumHelper.setDtwTripMcDmuAttributes(mcDmuObject, sampleMgra, destMgra,
                departPeriod, mcDmuObject.getDmuIndexValues().getDebug());

        logsumHelper.setWtwTripMcDmuAttributes(mcDmuObject, sampleMgra, destMgra, departPeriod,
                mcDmuObject.getDmuIndexValues().getDebug());

        logsum = logsumHelper.calculateTripMcLogsum(sampleMgra, destMgra, departPeriod, mcModel,
                mcDmuObject, nonManLogsumsLogger);
    ...
    {
        int returnValue = -1;
        if (index <= LAST_EA_INDEX) returnValue = EA;
        else if (index <= LAST_AM_INDEX) returnValue = AM;
        else if (index <= LAST_MD_INDEX) returnValue = MD;
        else if (index <= LAST_PM_INDEX) returnValue = PM;
        else returnValue = EV;
