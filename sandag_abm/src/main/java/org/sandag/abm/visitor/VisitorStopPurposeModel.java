...
/**
 * This class is the stop purpose choice model for visitor tours. It is
 ...
 *
 */
public class VisitorStopPurposeModel
{
    ...
    private void readStopPurposeFile(String fileName)
    {
        ...
        for (int row = 1; row <= rowCount; ++row)
        {
            ...
            if (Math.abs(cumProb - 1.0) > 0.00001)
                logger.info("Cumulative probability for tour purpose " + purpose + " inbound "
                        + inbound + " stopNumber " + stopNumber + " multiple " + multiple + " is "
                        + cumProb);
     ...
     */
    private int getKey(int tourPurp, int isInbound, int stopNumber, int multipleStopsOnLeg)
    {

        return tourPurp * 1000 + isInbound * 100 + stopNumber * 10 + multipleStopsOnLeg;
    }
     ...
     */
    public void calculateStopPurposes(VisitorTour tour)
    {
