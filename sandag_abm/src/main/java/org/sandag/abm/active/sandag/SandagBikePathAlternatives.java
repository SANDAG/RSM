...
public class SandagBikePathAlternatives
{
    ...
    public SandagBikePathAlternatives(
            PathAlternativeList<SandagBikeNode, SandagBikeEdge> pathAlternativeList)
    {
        this.pathAlternativeList = pathAlternativeList;
        this.network = (Network<SandagBikeNode, SandagBikeEdge, SandagBikeTraversal>) pathAlternativeList
                .getNetwork();
        ...
        for (int i = 0; i < getPathCount(); i++)
        {
            ...
            for (SandagBikeNode current : pathAlternativeList.get(i))
            {
                if (parent != null)
                {
                    ...
                    distClass1.set(i, distClass1.get(i) + edge.distance
                            * (edge.bikeClass == 1 ? 1 : 0));
                    distClass2.set(i, distClass2.get(i) + edge.distance
                            * (edge.bikeClass == 2 ? 1 : 0));
                    distClass3.set(i, distClass3.get(i) + edge.distance
                            * (edge.bikeClass == 3 ? 1 : 0));
                    distArtNoLane.set(i, distArtNoLane.get(i) + edge.distance
                            * (edge.bikeClass != 2 && edge.bikeClass != 1 ? 1 : 0)
                            * ((edge.functionalClass < 4 && edge.functionalClass > 0) ? 1 : 0));
                    distWrongWay.set(i, distWrongWay.get(i) + edge.distance
                            * (edge.bikeClass != 1 ? 1 : 0) * (edge.lanes == 0 ? 1 : 0));
                    distCycTrack.set(i, distCycTrack.get(i) + edge.distance
                            * (edge.cycleTrack ? 1 : 0));
                    ...
                    if (grandparent != null)
                    {
                        ...
                        signals.set(i, signals.get(i)
                                + (traversal.signalExclRightAndThruJunction ? 1 : 0));
    ...
    {
        return pathAlternativeList.getSizeMeasures().get(path)
                / pathAlternativeList.getSizeMeasureTotal();
    }
