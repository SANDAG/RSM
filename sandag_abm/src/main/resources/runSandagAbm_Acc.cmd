...
%JAVA_64_PATH%\bin\java -showversion -server -Xms%MEMORY_CLIENT_MIN% -Xmx%MEMORY_CLIENT_MAX% -cp "%CLASSPATH%" -Djxl.nowarnings=true -Dlog4j.configuration=log4j.xml -Dproject.folder=%PROJECT_DIRECTORY% -Djppf.config=jppf-client.properties -Djava.library.path=%JAR_LOCATION% org.sandag.abm.ctramp.CtrampApplication %PROPERTIES_NAME% -iteration %ITERATION% -luAcc false 2>&1 | tee.exe %PROJECT_DIRECTORY%\logFiles\build_accessibility_%ITERATION%.log
